...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (storedAvatar) {
      ...
      localStorage.setItem("selectedAvatar", defaultUserImage.src);
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (data.status === 200) {
        ...
        localStorage.setItem("userData", JSON.stringify(data.user)); // Fix: Stringify user data
        ...
        if (!storedAvatar) {
          localStorage.setItem("selectedAvatar", defaultUserImage.src); // Store image path
